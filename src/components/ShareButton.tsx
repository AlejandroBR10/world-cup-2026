import type { ShareButtonProps } from "../interfaces/share-button.interface";
...
function buildText(days: number, hours: number): string {
  ...
  if (hours > 0)
    return `¡Faltan solo ${hours} horas para el Mundial FIFA 2026! 🏆⚽ ¡Ya casi!`;
  return `¡El Mundial FIFA 2026 comienza en minutos! 🏆⚽🔥`;
}
...
export default function ShareButton({
  ...
  function shareWhatsApp() {
    const url = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${text} ${SHARE_URL}`)}`;
    window.open(url, "_blank", "noopener,noreferrer");
  }

  function shareFacebook() {
    const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(SHARE_URL)}&quote=${encodeURIComponent(text)}`;
    window.open(url, "_blank", "noopener,noreferrer");
  }

  // Móvil → botón único nativo
  if (canNativeShare) {
    return (
